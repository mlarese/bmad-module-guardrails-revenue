...
| --- | --- | --- |
| Sviluppo software | BMM Analyst/PM/Architect/Dev/TEA + Otto, Kai, Bruno, Enzo | `grv-board`, test, sicurezza, rollback |
| Database e persistenza | Dario + BMM Architect/Dev/TEA | glossario di dominio, ricerca live, benchmark, migrazione reversibile e `grv-board` |
...
| Sito WordPress, contenuti e consegna | Milo + Iris, Nora, Kai, Bruno secondo il segnale | `grl-wordpress-delivery`, modello contenuti approvato, media con attachment ID, autorizzazione esplicita e release gate di `grv-board` |
...
passo `grv-profile:domain` oppure marca il termine come `da confermare`: non trasformare una
...
| Se emerge | Passa a |
| --- | --- |
| codice, requisiti, test e release software | BMM e `grv-board` |
...
| modello dati, motore, schema, query, migrazione o recovery del datastore | Dario e `grl-agent-database`; `grv-profile:domain` se il linguaggio non è stabile |
